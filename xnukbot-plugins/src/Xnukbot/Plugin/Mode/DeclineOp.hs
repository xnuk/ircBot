...
import "text" Data.Text (isPrefixOf, isInfixOf)
...
plug setting send (Message (Just NickName{}) "MODE" (chan : mode : nicks))
    ...
    | "+" `isPrefixOf` mode && "o" `isInfixOf` mode && isJust nickIfMe = Just $ do
        let myNick = fromJust nickIfMe
        forkIO $ send $ (if "v" `isInfixOf` mode then [] else voice chan [myNick]) ++ deop chan [myNick]
        return setting
    | otherwise = Nothing
